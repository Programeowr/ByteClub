...
class EnableCores{
    ...
    bool checkStall(string instruction){
            ...
            if(opcode == "add" || opcode == "sub" || opcode == "mul" || opcode == "div"){
                ...
                if(!mem_wb.empty()){
                    ...
                    if((rd_two == rs1) && opcode_two != "sw" && opcode_two != "sw_spm"){
                        if(opcode == "bne" || opcode == "beq" || opcode == "blt" || opcode == "bge"){
                            r_rs1 = result_two;
                            branchStall = true;
                            branchDuration = 2;
                    ...
                    if((rd_two == rs2) && opcode_two != "sw" && opcode_two != "sw_spm"){
                        if(opcode == "bne" || opcode == "beq" || opcode == "blt" || opcode == "bge"){
                            r_rs2 = result_two;
                            branchStall = true;
                            branchDuration = 2;
                ...
                if(!ex_mem.empty()){
                    ...
                    if((rd_three == rs1 && rd_three == rs2) && (opcode_three != "sw") && (opcode_three != "sw_spm")){
                        if(opcode_three == "lw" || opcode_three == "lw_spm"){
                            stallDuration = 2;
                            return true;
                        }
                    ...
                    if((rd_three == rs1) && (opcode_three != "sw") && (opcode_three == "sw_spm")){
                        if(opcode_three == "lw" || opcode_three == "lw_spm"){
                            stallDuration = 2;
                            return true;
                        }
                    ...
                    if((rd_three == rs2) && (opcode_three != "sw") && (opcode_three != "sw_spm")){
                        if(opcode_three == "lw" || opcode_three == "lw_spm"){
                            stallDuration = 2;
                            return true;
                        }
    ...
    void memoryStage(vector<int>& memory, vector<vector<int>>& cacheOne, vector<vector<int>>& cacheTwo, vector<int>& scratchPadMem, int cacheOneSize, int cacheTwoSize, int spmSize, int blockSize, int associativityOne, int associativityTwo, char replacement, int cacheOneLatency, int cacheTwoLatency, int memoryLatency){
        ...
        if(ex_mem.empty())  return;
            ...
            else if(opcode == "lw_spm" || opcode == "sw_spm"){
                spmAccess++;
...
class EnableSimulator{
    ...
    void instructionFetch(vector<string>& program, int cid, int index) {
        ...
        if(cores[cid].stall || cores[cid].instructionStall > 0 || cores[cid].cacheStall > 0){
            return;
        } 
